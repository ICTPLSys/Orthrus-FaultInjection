...
constexpr uint32_t query_count = 5E4;
...
int main_fn() {
    init();
    profile::start();
    uint64_t thread_us_start = profile::thread_us();
    run();
    uint64_t thread_us_end = profile::thread_us();
    profile::stop();
    baseline();
    uint64_t thread_us_end_baseline = profile::thread_us();
    fprintf(stderr, "Thread time: %lu us\n", thread_us_end - thread_us_start);
    fprintf(stderr, "Baseline time: %lu us\n",
            thread_us_end_baseline - thread_us_end);
    return 0;
}
