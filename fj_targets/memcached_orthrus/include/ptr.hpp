...
public:
    ...
    void reref(const T *ptr) { store_ptr(this, ptr); }

    static ptr_t<T> *create() {
        auto *t = (ptr_t<T> *)alloc_ptr();
        store_ptr(t, nullptr);
        return t;
    }

    static ptr_t<T> *create(const T *ptr) {
        auto *t = (ptr_t<T> *)alloc_ptr();
        store_ptr(t, ptr);
        return t;
    }

    static ptr_t<T> *create(const T &obj) {
        auto *t = (ptr_t<T> *)alloc_ptr();
        const T *addr = make_obj(obj);
        store_ptr(t, addr);
        return t;
    ...
    static fixed_ptr_t<T> create_fixed(const T &obj) {
        return fixed_ptr_t<T>(make_obj(obj));
    }

    static const T *make_obj(const T &obj) {
