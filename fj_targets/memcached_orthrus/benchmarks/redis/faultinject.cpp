...
void ASSERT_EQ(const T& val1, const T& val2) {
    if ((val1) != (val2)) {
        fprintf(stderr, "Test failed: ASSERT_EQ(%d, %d)\n", val1, val2);
        abort();
    }
}
...
void final_check() {
    ...
    for (int i = 0; i < MaxCap; ++i) {
        ...
        while (entry != nullptr) {
            ...
            if (key_map.count(kk) == 0 || marked[key_map[kk]] != 0) {
                fprintf(stderr, "key: %s, key_map[kk]: %d, marked[key_map[kk]]: %d\n", kk.c_str(), key_map[kk], marked[key_map[kk]]);
                fprintf(stderr, "SDC Not Detected\n");
                abort();
            }

            if (vv != std::string(tmps[key_map[kk]], VAL_LEN)) {
                fprintf(stderr, "key: %s, vv: %s, tmps: %s\n", kk.c_str(),
                        vv.c_str(), tmps[key_map[kk]]);
                fprintf(stderr, "SDC Not Detected\n");
                abort();
            }
    ...
    if (marked_cnt != NKeys) {
        fprintf(stderr, "marked_cnt: %d, NKeys: %d\n", marked_cnt, NKeys);
        fprintf(stderr, "SDC Not Detected\n");
        abort();
    }
