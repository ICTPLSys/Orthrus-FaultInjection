...
int main_fn(const char *buffer, size_t file_size, map_reduce_config config) {
    ...
    profile::start();
#endif
    for (size_t k = 0; k < config.n_threads; ++k) {
        int client_fd = accept(listen_fd, nullptr, nullptr);
        assert(client_fd >= 0);
        mapper_threads.emplace_back([&, client_fd] {
            fd_reader reader(client_fd);
            size_t i, len = reader.read_packet('\x01');
            assert(len > 0);
            i = atoi(std::string(reader.packet, len - 1).c_str());
            std::string info = std::to_string(client_fd) + " " + std::to_string(len) + " " + std::to_string(i) + " ";
            info += std::string(reader.packet, len - 1); info += "\n";
...
#ifdef PROFILE
                profile::record_validation_latency((end_us - start_us) * kCpuMhzNorm / 1000);
    ...
    profile::stop();
#endif
    for (int j = 0; j < config.n_mappers * config.n_reducers; ++j) {
        auto it = map_results.deref(j);
        if (it->second == 0) {
            std::string info = std::to_string(j) + "\n";
            std::cerr << info << std::flush;
        }
    }
    ...
    for (size_t k = 0; k < config.n_threads; ++k) {
        ...
        reducer_threads.emplace_back([&, client_fd] {
            fd_reader reader(client_fd);
            size_t i, len = reader.read_packet('\x02');
            assert(len > 0);
            i = atoi(std::string(reader.packet, len - 1).c_str());
            for (size_t j = i; j < config.n_reducers; j += config.n_threads) {
                ...
                std::string s = std::string(reader.packet + 20, len - 21);
                rbv::hasher.deserialize(s);
                raw::word_count_reduce_worker(map_results, reduce_results, j,
                                              config);
                rbv::hasher.finalize();
                long long end_us = profile::get_us_abs();
#ifdef PROFILE
                profile::record_validation_latency(end_us - start_us);
                profile::record_validation_cpu_time(end_us - start_my_us, 1);
#endif
            }
