...
int main_fn(const char *buffer, size_t file_size, map_reduce_config config) {
    ...
    long long time_diff = 0;
    {
        ...
        // if check it at server side, add another RTT / 2
    ...
    for (size_t i = 0; i < config.n_threads; ++i) {
        mapper_threads.emplace_back([&, i] {
            ...
            assert(fd >= 0);
            write_all(fd, std::to_string(i) + "\x01");
            for (size_t j = i; j < config.n_mappers; j += config.n_threads) {
                ...
                std::string info = std::to_string(fd) + " " + std::to_string(j) + " " + std::to_string(start_us) + " " + s + "\n";
                std::cerr << info << std::flush;
            }
            fd_reader reader(fd); reader.read_packet('\x01');
        });
    }
    for (size_t i = 0; i < config.n_threads; ++i) {
    ...
    for (size_t i = 0; i < config.n_threads; ++i) {
        reducer_threads.emplace_back([&, i] {
            int fd = connect_server(config.replica_ip, config.replica_port + 1);
            assert(fd >= 0);
            write_all(fd, std::to_string(i) + "\x02");
            ...
            fd_reader reader(fd); reader.read_packet('\x02');
        });
    }
    for (size_t i = 0; i < config.n_threads; ++i) {
