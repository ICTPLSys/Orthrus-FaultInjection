...
void hash_table::inc(std::string_view key, size_t hash) {
    ...
    ::rbv::hasher.combine(key);
    ::rbv::hasher.combine(hash);
...
void hash_table::collect(kv_pair *kv_pairs) const {
    ...
    ::rbv::hasher.combine(size);
    for (size_t i = 0; i < size; ++i) {
        ::rbv::hasher.combine(kv_pairs[i].key.hash());
        ::rbv::hasher.combine(kv_pairs[i].value);
    }
}
...
static size_t scan_words(const char *start, size_t length, Fn add_kv) {
    ::rbv::hasher.combine(length);
    const char *end = start + length;
    const char *cursor = skip_spaces(start, end);
    while (cursor < end) {
        ::rbv::hasher.combine(cursor - start);
...
void word_count_map_worker(std::string_view input,
    ...
    ::rbv::hasher.combine(config.n_reducers);
    ::rbv::hasher.combine(config.n_buckets);
    ::rbv::hasher.combine(mapper_idx);
...
static void shuffle_kv_pairs(kv_pair *kv_pairs, size_t size) {
    std::sort(kv_pairs, kv_pairs + size,
              [](const kv_pair &a, const kv_pair &b) {
                  return key_less(a.key, b.key) || (key_equal(a.key, b.key) && a.value < b.value);
              });
}
...
static result_t reduce(const kv_pair *kv_pairs, size_t size) {
    ...
    auto reduced_arr = reduced.deref();
    for (size_t i = 0; i < reduced_size; ++i) {
        ::rbv::hasher.combine(reduced_arr[i].key.hash());
        ::rbv::hasher.combine(reduced_arr[i].value);
    }
...
void word_count_reduce_worker(scee::mut_array<result_t> map_results,
    ...
    ::rbv::hasher.combine(reducer_idx);
    // collect all kv pairs from all mappers
    size_t total_kvs = 0;
    for (size_t i = 0; i < config.n_mappers; ++i) {
        auto it = get_ith_mapper_result(i);
        std::string info = std::to_string(reducer_idx) + " " + std::to_string(i) + " " + std::to_string(it->second) + "\n";
        std::cerr << info << std::flush;
        ::rbv::hasher.combine(it->second);
        total_kvs += it->second;
    }
    ...
    for (size_t i = 0; i < config.n_mappers; ++i) {
        ...
        for (size_t j = 0; j < n_kvs; ++j) {
            kv_pairs[kv_offset + j] = src_kv_pairs[j];
            ::rbv::hasher.combine(src_kv_pairs[j].key.hash());
            ::rbv::hasher.combine(src_kv_pairs[j].value);
        }
        // combine input kvpairs
    ...
    imm_array<kv_pair> final_results = imm_array<kv_pair>::create(
        total_size, [results, total_size, n_reducers](kv_pair *final_results) {
            ...
            std::sort(final_results, final_results + total_size,
                      [](const kv_pair &a, const kv_pair &b) {
                          return a.value > b.value || (a.value == b.value && key_less(a.key, b.key));
                      });
        });
