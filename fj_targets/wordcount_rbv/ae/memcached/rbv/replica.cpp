...
thread_local long long timestamp;
...
struct fd_worker {
    ...
    fd_worker(int _fd) : reader(_fd) {
        ...
        assert(len > 0);
        long long client_us = atoll(std::string(reader.packet, len - 1).c_str());
        long long server_us = profile::get_us_abs();
        write_all(_fd, std::to_string(server_us) + "\x01");
    }
    ...
    void run() {
        while ((len = reader.read_packet('\x01'))) {
            ...
            profile::record_validation_cpu_time(end_us - timestamp, 1);
#endif
        }
    }
...
void Start(int port) {
    ...
    timestamp = profile::get_us_abs();
    while (profile::get_us_abs() - timestamp < 10'000'000) {
        worker.run();
    }
}
...
int main(int argc, char *argv[]) {
    if (argc < 1 || argc > 4) {
        ...
        fprintf(stderr, "Default values: port=6789, ngroups=3, log_file=server.log\n");
        return 1;
    }
    uint32_t port = 6789;
