...
inline hash_table::~hash_table() { delete[] entries; }
...
// accept output from sort_results as input.
void calculate_word_frequency(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    const auto *fr = final_results.deref_nullable();

    size_t total_size = 0;

    for (size_t i = 0; i < final_result_size; ++i) {
        total_size += fr[i].value;
    }

    for (size_t i = 0; i < std::min<size_t>(final_result_size, 10); ++i) {
        if (key_equal(fr[i].key, std::string_view("seven"))) {
            if (fr[i].value != 7) {
                printf("key size is not 7 for 'seven'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("six"))) {
            if (fr[i].value != 6) {
                printf("key size is not 6 for 'six'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("five"))) {
            if (fr[i].value != 5) {
                printf("key size is not 5 for 'five'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("four"))) {
            if (fr[i].value != 4) {
                printf("key size is not 4 for 'four'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("three"))) {
            if (fr[i].value != 3) {
                printf("key size is not 3 for 'three'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("two"))) {
            if (fr[i].value != 2) {
                printf("key size is not 2 for 'two'");
                printf("sdc error detected\n");
                abort();
            }
        } else if (key_equal(fr[i].key, std::string_view("one"))) {
            if (fr[i].value != 1) {
                printf("key size is not 1 for 'one'");
                printf("sdc error detected\n");
                abort();
            }
        }
            printf("%8.*s: %lu\t%f\t", (int)fr[i].key.size, fr[i].key.data.deref(),
                fr[i].value, static_cast<double>(fr[i].value) / total_size);
    }
    printf("\n");
    ...
    for (size_t i = 0; i < final_result_size; ++i) {
        total_length += fr[i].key.size * fr[i].value;
    }

    printf("total length: %zu, total size: %zu\n", total_length, total_size);
    double average_length = static_cast<double>(total_length) / total_size;
    printf("average length of the words: %lf\n", average_length);
    if (average_length > 4.1 || average_length < 3.9) {
        printf("average length of the words is not 4.0");
        printf("sdc error detected\n");
        abort();
    }

    // calculate the average frequency of the words
    double average_frequency = static_cast<double>(total_size) / total_size;
    printf("average frequency of the words: %lf\n", average_frequency);
    if (average_frequency != 1.0) {
        printf("average frequency of the words is not 1.0");
        printf("sdc error detected\n");
        abort();
    }

    // calculate the standard deviation of the words
    double standard_deviation = 0;
    for (size_t i = 0; i < final_result_size; ++i) {
        standard_deviation += (fr[i].value - average_frequency) * (fr[i].value - average_frequency);
    }
    standard_deviation = std::sqrt(standard_deviation / total_size);
    printf("standard deviation of the words: %lf\n", standard_deviation);
    if (standard_deviation > 1.81 || standard_deviation < 1.79) {
        printf("standard deviation of the words is not 1.80");
        printf("sdc error detected\n");
        abort();
    }

    // calculate the proportion of the words that have length 5
}

size_t calc_total_size(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    const auto *fr = final_results.deref_nullable();
    size_t total_size = 0;
    for (size_t i = 0; i < final_result_size; ++i) {
        total_size += fr[i].value;
...
__attribute_noinline__ double calc_each_word_frequency(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    word_frequency wf;
    
    const auto *fr = final_results.deref_nullable();
    size_t total_size = calc_total_size(final_results, final_result_size);

    for (size_t i = 0; i < final_result_size; ++i) {
        wf.key[i] = fr[i].key;
        wf.frequency[i] = static_cast<double>(fr[i].value) / total_size;
    }

    return wf.frequency[final_result_size - 1];
}

__attribute_noinline__ double calc_average_length(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    const auto *fr = final_results.deref_nullable();
    size_t total_size = calc_total_size(final_results, final_result_size);
    ...
    return static_cast<double>(total_length) / total_size;
}

__attribute_noinline__ double calc_average_frequency(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    return static_cast<double>(calc_total_size(final_results, final_result_size)) / final_result_size;
}

__attribute_noinline__ double calc_standard_deviation(scee::imm_array<kv_pair> final_results, size_t final_result_size) {
    size_t total_size = calc_total_size(final_results, final_result_size);
    double average_frequency = calc_average_frequency(final_results, final_result_size);
    double standard_deviation = 0;
    const auto *fr = final_results.deref_nullable();
    for (size_t i = 0; i < final_result_size; ++i) {
        standard_deviation += (fr[i].value - average_frequency) * (fr[i].value - average_frequency);
    }
    return std::sqrt(standard_deviation / total_size);
}
