...
class imm_array {
private:
    ptr_t<T> *ptr;  // point to first element of the array

private:
    explicit imm_array(const T *ptr) : ptr(ptr_t<T>::create(ptr)) {}
...
public:
    ...
    static imm_array<T> create(size_t size, Fn init) {
        T *arr = (T *)alloc_obj(size * sizeof(T));
    ...
    const T *deref() const { return ptr->load_logless(); }

    const T *deref_nullable() const {
        return ptr ? ptr->load_logless() : nullptr;
    }
};
...
public:
    static mut_array<T> create(size_t size) {
        ...
        memset(arr, 0, size * sizeof(ptr_t<T>));
