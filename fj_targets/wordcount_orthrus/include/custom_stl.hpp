...
public:
    T *v;
    imm_array_t(void *data, size_t length) {
        size = sizeof(*this) + length * sizeof(T);
        this->v = (T *)data;
    }
    size_t length() const { return (size - sizeof(*this)) / sizeof(T); }
    void write_at(void *shadow, void *real) const {
        ...
        *(imm_array_t<T> *)shadow = imm_array_t<T>(real_data_ptr, length());
        memcpy(shadow_data_ptr, this->v, length() * sizeof(T));
    }
};
...
public:
    ...
    size_t length() const { return imm_array_t<local_ptr_t>::length(); }
    void write_at(void *shadow, void *real) const {
        void *real_data_ptr = add_byte_offset(real, sizeof(*this));
        void *shadow_data_ptr = add_byte_offset(shadow, sizeof(*this));
        *(mut_array_t<T> *)shadow = mut_array_t<T>(real_data_ptr, length());
