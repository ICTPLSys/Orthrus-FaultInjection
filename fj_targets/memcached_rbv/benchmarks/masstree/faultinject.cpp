...
#define sdc_assert(condition)                                              \
    if (!(condition)) {                                                    \
        fprintf(stderr, "SDC failure at %s:%d (%s)\n", __FILE__, __LINE__, \
                #condition);                                               \
        std::abort();                                                      \
    }

constexpr int kNThreads = 8, kNKeysPerThread = 20;
...
void prepare_dataset() {
    ...
    for (int i = 0; i < kNThreads; ++i) {
        for (int j = 0; j < kNKeysPerThread; ++j) {
            keys[i][j] =
                gen() % 100000 + 1;  // (uint64_t(gen()) << 32) ^ gen();
...
void traverse_tree(const Node *node, const Node *parent) {
    assert(!(node->occ->ver.load(std::memory_order_relaxed) &
             OCCControl::DELETED));
    ...
    if (node->node_type == NodeType::LEAF) {
        ...
        for (int i = 0; i < data->num_keys; ++i) {
            sdc_assert(data->sorted[i] < data->num_keys &&
                       data->sorted[i] >= 0);
            if (leaf_keys.size() > 0) {
                sdc_assert(kv_map.find(data->keys[data->sorted[i]]) !=
                           kv_map.end());
                const Value *val = *(Value **)(data->links[data->sorted[i]]);
                sdc_assert(val != nullptr);
                // fprintf(stderr, "CHK key: %lu, val: %s\n",
                // data->keys[data->sorted[i]], std::string(val->ch,
                // VAL_LEN).c_str());
                sdc_assert(memcmp(val->ch,
                                  kv_map[data->keys[data->sorted[i]]].ch,
                                  VAL_LEN) == 0);
            }
        ...
        for (int i = 0; i < data->num_keys; ++i) {
            sdc_assert(data->sorted[i] < data->num_keys &&
                       data->sorted[i] >= 0);
            const Node *child =
                static_cast<const Node *>(data->links[data->sorted[i]]);
            const Node::Data *child_data = *(Node::Data **)child->data;
            sdc_assert(child_data->keys[child_data->sorted[0]] ==
                       data->keys[data->sorted[i]]);
...
void final_check() {
    ...
    for (size_t i = 0; i < leaf_nodes.size(); ++i) {
        ...
        const LeafNode *exp_next =
            i < leaf_nodes.size() - 1 ? leaf_nodes[i + 1] : nullptr;
    ...
    for (int t = 0; t < do_not_inject::kNThreads; ++t) {
        threads.emplace_back([t]() {
            for (int i = 0; i < do_not_inject::kNKeysPerThread; ++i) {
                ...
                // std::string(val.ch, VAL_LEN).c_str());
                using InsertType =
