...
int main_fn(RunType rt, uint32_t port, int num_servers) {
...
#ifdef WITH_MONITOR
    FILE *logger =
        fopen(("build/server-" + std::to_string(rt) + ".log").c_str(), "w");
    mem_monitor = new monitor::process_memory(logger, num_servers * 2);
    cpu_monitor = new monitor::process_cpu(logger, num_servers * 2);
#endif
    ...
    for (int i = 0; i < num_servers; ++i) {
        ...
        case RunType::Baseline:
            normal_threads.emplace_back(
                [port, i]() { Start<RunType::Baseline>(port + i); });
            break;
        case RunType::SCEE:
            app_threads.emplace_back(
                [port, i]() { Start<RunType::SCEE>(port + i); });
            break;
        case RunType::SCEEProfile:
            app_threads.emplace_back(
                [port, i]() { Start<RunType::SCEEProfile>(port + i); });
            break;
        }
    }
...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 5) {
        fprintf(stderr,
                "Usage: %s [baseline|scee|scee-profile] <port> [num_servers]\n",
                argv[0]);
        return 1;
    }
    RunType rt;
