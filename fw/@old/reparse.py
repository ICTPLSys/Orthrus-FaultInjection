...
def main(args):
    ...
    for fname, pc_fj_info_list in data.items():
        ...
        for pc_fj_info in pc_fj_info_list:
            ...
            # Reparse the error type for output
            if "out" in result['injection']["data"]:
                res_injection_out = result['injection']["data"]["out"]
                test_failed_pat = re.compile(r"(.+\..+:\d+): Failure")
                for line in res_injection_out:
                    if (matches := test_failed_pat.match(line)):
                        result['injection']['error'] = "TestFailed"
                        result['injection']["data"] = matches.group(0)
                        break
