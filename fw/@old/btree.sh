...
worker() {
    set +e
    find /dev/shm -exec rm {} \;
    set -e

    fault_name=$1
    fault_template=$2

    result_file=$OUT_DIR/results.btree.$fault_name.json
    parse_file=$OUT_DIR/parse.btree.$fault_name.log
    ...
    time python3 test.py \
        --test-exec tree/btree_test \
        --scee-dir ../scee_asplos_btree \
        --threads 16 \
